...
app.get('/api/character-image/:id', (req, res) => {
    try {
        ...
        `);
        const result: { image_data: Buffer | null, image_content_type: string | null, image_fetched_at: string | null } | undefined = stmt.get(characterId);

        const sevenDaysAgo = new Date();
        sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);

        if (result && result.image_data && new Date(result.image_fetched_at) > sevenDaysAgo) {
            res.setHeader('Content-Type', result.image_content_type);
            res.setHeader('Cache-Control', 'public, max-age=604800');
            res.send(result.image_data);
        } else {
            fetch(`https://images.evetech.net/characters/${characterId}/portrait?tenant=tranquility&size=64`)
                .then(fetchRes => {
